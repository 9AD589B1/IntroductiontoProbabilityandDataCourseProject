...
BRFSS <- load ( url ( "https://d18ky98rnyall9.cloudfront.net/_384b2d9eda4b29131fb681b243a7767d_brfss2013.RData?Expires=1506816000&Signature=f6F5RNONXaCBBZwRtL7-mvVgbKHFUBSJVq5~eHnKfLYC0kxH7LwmPIVrg4RoWztBDV5xvHB25tt6Vvpr4me5bfJ6LKhyUOIBRP0bUH5QPolkth0G9b4DTrui3uojAFEAEKB5Xi3dZcMlmfseaYtzxx0n3ZFPSlR7R5jdc-m4tSI_&Key-Pair-Id=APKAJLTNE6QMUY6HBC5A" ) )
...
Since data collection is limited to telephone interviews & institutionalized individuals, and the questions are health-related, several biases could be introduced into the data:
...
brfss2013Sub1 <- brfss2013 %>%
    ...
    mutate ( CoronaryHeartDiseaseRank = case_when ( cvdcrhd4 == "Yes" ~ "Diagnosed", cvdcrhd4 == "No" ~ "Not Diagnosed", TRUE ~ "Unknown" ), 
             CalculatedSmokingRank = case_when ( X_smoker3 == "Never smoked" ~ 1, X_smoker3 == "Former smoker" ~ 2, X_smoker3 == "Current smoker - now smokes some days" ~ 3, X_smoker3 == "Current smoker - now smokes every day" ~ 4, TRUE ~ 0 )) %>%
...
brfss2013Sub1Summary3 <- brfss2013Sub1Summary2 %>%
    ...
    mutate ( DiagnosedPivot = case_when ( CoronaryHeartDiseaseRank == "Diagnosed" ~ Total ), NotDiagnosedPivot = case_when ( CoronaryHeartDiseaseRank == "Not Diagnosed" ~ Total ) ) %>%
    group_by ( Region ) %>%
    summarise ( Diagnosed = max(DiagnosedPivot, na.rm = TRUE), NotDiagnosed = max(NotDiagnosedPivot, na.rm = TRUE) ) %>%
    mutate ( Diagnosed = paste( round (Diagnosed, digits = 3 )* 100, "%", sep = "" ), NotDiagnosed = paste( round (NotDiagnosed, digits = 3 ) * 100, "%", sep = "" ) ) %>%
...
**Plot scatterplots of Hours of Sleep vs. Number of Days of Poor Mental Health By Depression Felt**
```{r}
...
We can see a slight negative correlation between hours of sleep Poor Mental Health Not Good Days, which is expected. What's interesting is the correlation is comparatively weaker when depression was felt on All" days, except on "Most" days when the correlation is ironically positive.
...
A drinking seasonality effect appears only to affect people of lower education, where there are spikes in late summer and early winter for those that Never attended school or only Kindergarten. What's really striking is the dramatic difference in average consumption between those with 4 or more years of college and those that never attended school, and volatility of drinking trends as education decreases.
